...
inline auto token_type_to_string(token_type t) {
    switch (t) {
        ...
        case token_type::NULL_: return "NULL";
        ...
        case token_type::LESS_THAN_OR_EQUAL: return "LESS_THAN_OR_EQUAL";
        case token_type::GREATER_THAN_OR_EQUAL: return "GREATER_THAN_OR_EQUAL";
...
token::token(const std::string& lexeme, token_type type): _lexeme(lexeme), _type(type)
{}
...
void lexer::read_in_token_types(){
    token_patterns = {
        ...
        {std::regex(R"([a-z_][a-z0-9_]*)", std::regex_constants::icase), token_type::IDENTIFIER},
...
auto lexer::content_to_tokens() -> std::vector<token>{
    ...
    size_t i = 0;
     ...
     while (i < content.size()) {
        ...
        if (!matched) {
            std::cerr << "Unexpected character: " << content[i] << " at position " << i << "\n";
